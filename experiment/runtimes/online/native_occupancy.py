"""ctypes wrapper for native causal free/occupied/unknown sweep queries."""
...
from incremental_occupancy_map import (
    FREE,
    OCCUPIED,
    UNKNOWN,
    MapUpdateStats,
...
class NativeIncrementalOccupancyMap:
    """Native causal map builder with the reference map's exact update rules."""
    ...
    def __init__(
        ...
        *,
        ...
        self._lib.occ_incremental_create.argtypes = [
            ctypes.c_double,
            ctypes.c_int32,
            ctypes.c_int32,
            ctypes.c_int32,
        ...
        self._lib.occ_incremental_set_state_journal_enabled.argtypes = [
            vp, ctypes.c_int32
        ]
        ...
        self._lib.occ_incremental_integrate.argtypes = [
            vp, dp, dp, dp, i8p, ctypes.c_int32, i64p
        ]
        self._lib.occ_incremental_integrate.restype = ctypes.c_int32
        self._lib.occ_incremental_integrate_ellipsoids.argtypes = [
            vp, dp, dp, dp, dp, i8p, ctypes.c_int32, i64p
        ]
        self._lib.occ_incremental_integrate_ellipsoids.restype = ctypes.c_int32
        self._lib.occ_incremental_mark_free.argtypes = [
            vp, dp, dp, ctypes.c_int32, ctypes.c_double
        ]
        self._lib.occ_incremental_mark_free.restype = ctypes.c_int32
        self._lib.occ_incremental_certify_swept_spheres.argtypes = [
            vp,
        ...
        self._lib.occ_incremental_export.argtypes = [
            vp, i32p, i8p, ctypes.c_int32
        ]
        ...
        self._lib.occ_incremental_export_dirty.argtypes = [
            vp, i32p, i8p, ctypes.c_int32
        ]
        self._lib.occ_incremental_export_dirty.restype = ctypes.c_int32
        self._handle = self._lib.occ_incremental_create(
            float(voxel_size),
            int(free_decrement),
            int(occupied_increment),
            int(minimum_score),
            int(maximum_score),
        ...
        code = self._lib.occ_incremental_set_state_journal_enabled(
            self._handle, int(bool(track_state_deltas))
        )
        if code != 0:
            self.close()
    ...
    def set_calibrated_free_aabb(
        self, lower: np.ndarray, upper: np.ndarray
    ) -> None:
        """Install a fixed staging volume; observed OCCUPIED always wins."""

        lower = np.ascontiguousarray(lower, dtype=np.float64).reshape(3)
        upper = np.ascontiguousarray(upper, dtype=np.float64).reshape(3)
        dp = ctypes.POINTER(ctypes.c_double)
        code = self._lib.occ_incremental_set_calibrated_free_aabb(
            self._handle,
            lower.ctypes.data_as(dp),
            upper.ctypes.data_as(dp),
        )
        if code != 0:
            raise ValueError(f"invalid calibrated free AABB (code {code})")

    def integrate(
        self, observations: Iterable[DepthObservation]
    ) -> MapUpdateStats:
        observations = list(observations)
        point_blocks = [
            np.asarray(item.points, dtype=np.float64).reshape(-1, 3)
        ...
        for item, points in zip(observations, point_blocks):
            ...
            if (
                item.sample_uncertainty_shapes is not None
                and len(item.sample_uncertainty_shapes) != len(points)
            ):
                raise ValueError(
                    "each depth point needs one directional uncertainty shape"
                )
        directional = bool(observations) and all(
            item.sample_uncertainty_shapes is not None for item in observations
        )
        ...
        if ray_count:
            ...
            radii = np.ascontiguousarray(
                np.concatenate([item.sample_radii for item in observations]),
                dtype=np.float64,
            )
            endpoint_masks = np.ascontiguousarray(
                np.concatenate(
                    [environment_endpoint_mask(item) for item in observations]
                ),
                dtype=np.int8,
            )
            origins = np.ascontiguousarray(
                np.vstack(
                    [
                        np.repeat(
                            np.asarray(item.camera_position, dtype=np.float64)[None, :],
                            len(points),
                            axis=0,
                        )
                        for item, points in zip(observations, point_blocks)
            ...
            shapes = (
                np.ascontiguousarray(
                    np.vstack(
                        [item.sample_uncertainty_shapes for item in observations]
                    ),
                    dtype=np.float64,
                ).reshape(-1, 3, 3)
                if directional
        ...
        dp = ctypes.POINTER(ctypes.c_double)
        if directional:
            code = self._lib.occ_incremental_integrate_ellipsoids(
                self._handle,
                origins.ctypes.data_as(dp),
                endpoints.ctypes.data_as(dp),
                radii.ctypes.data_as(dp),
                shapes.ctypes.data_as(dp),
                endpoint_masks.ctypes.data_as(ctypes.POINTER(ctypes.c_int8)),
                ray_count,
                self._stats.ctypes.data_as(ctypes.POINTER(ctypes.c_int64)),
            )
        else:
            code = self._lib.occ_incremental_integrate(
                self._handle,
                origins.ctypes.data_as(dp),
                endpoints.ctypes.data_as(dp),
                radii.ctypes.data_as(dp),
        ...
        return MapUpdateStats(
            frame_index=int(self._stats[0]),
            rays=int(self._stats[1]),
            free_updates=int(self._stats[2]),
            occupied_updates=int(self._stats[3]),
            free_voxels=int(self._stats[4]),
            occupied_voxels=int(self._stats[5]),
    ...
    def mark_current_robot_free(
        ...
        dp = ctypes.POINTER(ctypes.c_double)
        code = self._lib.occ_incremental_mark_free(
            self._handle,
    ...
    def certify_swept_spheres(
        ...
        *,
        ...
        code = self._lib.occ_incremental_certify_swept_spheres(
            self._handle,
            starts.ctypes.data_as(fp),
            ends.ctypes.data_as(fp),
            radii.ctypes.data_as(fp),
            len(radii),
            float(margin),
            self._query_counts.ctypes.data_as(ctypes.POINTER(ctypes.c_int32)),
        )
        if code < 0:
            raise RuntimeError(
                f"native incremental occupancy query failed with code {code}"
            )
        self._unknown_queries += int(self._query_counts[1])
        return SweptVolumeCertificate(
            safe=bool(code),
            occupied_voxels=int(self._query_counts[0]),
            unknown_voxels=int(self._query_counts[1]),
            checked_voxels=int(self._query_counts[2]),
        )
    ...
    def snapshot_arrays(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        written = self._lib.occ_incremental_export(
            self._handle,
            keys.ctypes.data_as(ctypes.POINTER(ctypes.c_int32)),
            states.ctypes.data_as(ctypes.POINTER(ctypes.c_int8)),
            count,
        )
        if written != count:
            raise RuntimeError(
                f"native occupancy export wrote {written}, expected {count}"
            )
        return keys, states

    def snapshot_delta_arrays(self) -> tuple[np.ndarray, np.ndarray]:
        """Export discrete state changes and atomically advance the journal."""

        if not self.track_state_deltas:
            raise RuntimeError(
                "state delta journal is disabled; construct with track_state_deltas=True"
            )
        ...
        keys = np.empty((count, 3), dtype=np.int32)
        states = np.empty(count, dtype=np.int8)
        written = self._lib.occ_incremental_export_dirty(
            self._handle,
            keys.ctypes.data_as(ctypes.POINTER(ctypes.c_int32)),
            states.ctypes.data_as(ctypes.POINTER(ctypes.c_int8)),
            count,
        ...
        if written != count:
            raise RuntimeError(
                f"native occupancy delta export wrote {written}, expected {count}"
            )
        return keys, states
    ...
    def clone_snapshot(self) -> "NativeOccupancySnapshot":
        ...
        if not handle:
            raise RuntimeError(
                "native occupancy clone failed (calibrated AABB snapshots are unsupported)"
            )
        return NativeOccupancySnapshot._from_native_handle(
            self._lib, handle, self.voxel_size
        )

    def state_counts(self) -> dict[int, int]:
        _, states = self.snapshot_arrays()
        return {
            UNKNOWN: int(np.count_nonzero(states == UNKNOWN)),
            FREE: int(np.count_nonzero(states == FREE)),
            OCCUPIED: int(np.count_nonzero(states == OCCUPIED)),
        }
...
class NativeOccupancySnapshot:
    ...
    def _configure_library(lib) -> None:
        ...
        lib.occ_certify_swept_spheres.argtypes = [
            ctypes.c_void_p,
            fp,
            fp,
            fp,
        ...
        dp = ctypes.POINTER(ctypes.c_double)
        lib.occ_certify_ellipsoids.argtypes = [
            ctypes.c_void_p,
            dp,
            dp,
            ctypes.c_int32,
    ...
    def _from_native_handle(
        cls, lib, handle, voxel_size: float
    ) -> "NativeOccupancySnapshot":
    ...
    def __init__(self, keys: np.ndarray, states: np.ndarray, voxel_size: float):
        ...
        i32p = ctypes.POINTER(ctypes.c_int32)
        i8p = ctypes.POINTER(ctypes.c_int8)
        self._handle = self._lib.occ_create(
            keys.ctypes.data_as(i32p),
            states.ctypes.data_as(i8p),
            len(keys),
            float(voxel_size),
        )
        if not self._handle:
    ...
    def certify_swept_spheres(
        ...
        *,
        ...
        if starts.shape != ends.shape or len(starts) != len(radii):
            raise ValueError("swept sphere arrays need matching lengths")
        fp = ctypes.POINTER(ctypes.c_float)
        code = self._lib.occ_certify_swept_spheres(
            self._handle,
            starts.ctypes.data_as(fp),
            ends.ctypes.data_as(fp),
            radii.ctypes.data_as(fp),
        ...
        return SweptVolumeCertificate(
            safe=bool(code),
            occupied_voxels=int(self._counts[0]),
            unknown_voxels=int(self._counts[1]),
            checked_voxels=int(self._counts[2]),
        )

    def certify_ellipsoids(
        self, centers: np.ndarray, shapes: np.ndarray
    ) -> SweptVolumeCertificate:
        """Certify outer ellipsoids against full occupied/unknown voxel boxes."""
        ...
        dp = ctypes.POINTER(ctypes.c_double)
        code = self._lib.occ_certify_ellipsoids(
            self._handle,
            centers.ctypes.data_as(dp),
            shapes.ctypes.data_as(dp),
            len(centers),
