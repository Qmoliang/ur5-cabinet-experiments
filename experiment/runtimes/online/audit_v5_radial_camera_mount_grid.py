"""Visibility/collision-only radial forearm D405 mount screen for v5."""
...
def main() -> None:
    ...
    rows = []
    # Frozen broad physical family: 12 circumferential sites, three axial
    # sites, nine drawer-front targets and three optical-axis rolls.
    for angle_deg in range(0, 360, 30):
        angle = math.radians(angle_deg)
        for local_z in (0.25, 0.31, 0.37):
            local_position = np.array(
                [0.075 * math.sin(angle), 0.075 * math.cos(angle), local_z]
            )
            for target_y_offset in (-0.10, 0.0, 0.10):
                for target_z_offset in (-0.10, -0.05, 0.0):
                    target = np.array(
                        [
                            0.35,
                    ...
                    for roll in (-30.0, -15.0, 0.0):
                        ...
                        rows.append(
                            {
                                "radial_angle_deg": angle_deg,
                                "forearm_local_position_m": (
                                ...
                                "collision_free_on_both_trajectories": (
                                    collision_free
                                ),
                                "collision": collision,
    ...
    rows.sort(
        key=lambda row: (
            ...
            row["radial_angle_deg"],
            row["forearm_local_position_m"][2],
    ...
    report = {
        "audit_role": "protocol_v5_two_physical_d405_radial_screen",
