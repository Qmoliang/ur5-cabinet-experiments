"""Extracted original v4.4 sphere dependency; see docs/source_manifest.json."""
...
class IncrementalMatchedProxyManager:
    ...
    def __init__(self, *, filter_size: float=0.006, cluster_size: float=0.1, maximum_aabb_overshoot: float=0.025, maximum_uncertainty_union_inflation: float | None=None, normal_connection_distance: float | None=None, certificate_radius_limit: float | None=None, uncertainty_fusion_mode: str='separate_uncertainty', direct_thin_axis_inflation: float=np.sqrt(2.0), direct_tangent_subdivisions: int=1, direct_partition_mode: str='grid', sphere_cover_mode: str='matched', origin: np.ndarray=np.array([-1.2, -1.2, 0.0])) -> None:
        ...
        if sphere_cover_mode not in {'matched', 'adaptive_irredundant'}:
            raise ValueError('unknown sphere cover mode')
        self.sphere_cover_mode = sphere_cover_mode
        if self.sphere_cover_mode == 'adaptive_irredundant':
            if self.certificate_radius_limit is None:
                raise ValueError('adaptive irredundant spheres require a candidate radius limit')
    ...
    def _publish_adaptive_irredundant_sphere_snapshot(self, ordered_keys: list[tuple[int, int, int]], candidate_centers: np.ndarray, candidate_radii: np.ndarray) -> PointCloudProxySet:
        if self.certificate_radius_limit is None:
        ...
        cell_centers = np.asarray([cell.representative for cell in cells], dtype=float)
        if any((cell.uncertainty_shape is None for cell in cells)):
            raise AssertionError('adaptive directional sphere cover lost CenterVox U')
        cell_shapes = np.asarray([cell.uncertainty_shape for cell in cells], dtype=float)
        cell_support_radii = np.sqrt(np.maximum(np.linalg.eigvalsh(cell_shapes)[:, -1], 0.0)) + np.asarray([cell.cover_radius for cell in cells], dtype=float)
        if float(np.max(cell_support_radii)) > cap + 1e-12:
            index = int(np.argmax(cell_support_radii))
            raise ValueError(f'one indivisible CenterVox certificate exceeds the sphere candidate radius: key={ordered_keys[index]}, required={cell_support_radii[index]:.9g}, limit={cap:.9g}')
        ...
        if np.any(candidate_radii <= 0.0) or np.any(candidate_radii > cap + 1e-09):
        ...
        for candidate_center, candidate_radius in zip(candidate_centers, candidate_radii):
            neighbours = np.asarray(sorted(tree.query_ball_point(candidate_center, candidate_radius)), dtype=np.int64)
            required = np.linalg.norm(candidate_center[None, :] - cell_centers[neighbours], axis=1) + cell_support_radii[neighbours]
            keep = required <= candidate_radius + 1e-12
        ...
        for position in range(len(selected) - 1, -1, -1):
            ...
            if np.all(cover_counts[coverage[candidate]] >= 2):
                ...
                reverse_deleted += 1
        retained_candidates = np.asarray(sorted((selected[position] for position in range(len(selected)) if retained_mask[position])), dtype=np.int64)
        ...
        for proxy_index, candidate in enumerate(retained_candidates):
            ...
            better = required < owner_required[indices] - 1e-15
            ties = np.abs(required - owner_required[indices]) <= 1e-15
        ...
        if np.any(final_radii <= 0.0) or np.any(final_radii > cap + 1e-12):
        ...
        return PointCloudProxySet(raw_points=cell_centers.copy(), filtered_points=cell_centers.copy(), centers=centers, sphere_radii=final_radii, ellipsoid_shapes=shapes.copy(), rotations=identity, cluster_keys=cluster_keys, filtered_cluster_indices=owner, maximum_ellipsoid_overshoot=0.0, surface_cover_radius=float(np.max(final_radii)), proxy_offset_radii=np.zeros(len(centers), dtype=float), filtered_uncertainty_shapes=cell_shapes, proxy_uncertainty_shapes=zeros, base_sphere_radii=final_radii.copy(), base_ellipsoid_shapes=shapes.copy(), ellipsoid_outer_shapes=shapes.copy(), maximum_uncertainty_union_inflation=1.0, certificate_radius_limit=cap, uncertainty_fusion_mode=self.uncertainty_fusion_mode, sphere_cover_mode=self.sphere_cover_mode, sphere_cover_candidate_count=candidate_count, sphere_cover_selected_count=len(centers), sphere_cover_reverse_deleted=reverse_deleted)

    def _build_resolution_bounded_sphere_candidates(self, ordered_keys: list[tuple[int, int, int]], fitted: PointCloudProxySet) -> tuple[np.ndarray, np.ndarray]:
        cells = [self._cells[key] for key in ordered_keys]
        centers = np.asarray([cell.representative for cell in cells], dtype=float)
        shapes = np.asarray([cell.uncertainty_shape for cell in cells], dtype=float)
        support = np.sqrt(np.maximum(np.linalg.eigvalsh(shapes)[:, -1], 0.0)) + np.asarray([cell.cover_radius for cell in cells], dtype=float)
        assignments = np.asarray(fitted.filtered_cluster_indices, dtype=np.int64)
        ...
        def refine(indices: np.ndarray, depth: int=0) -> None:
            ...
            required = float(np.max(np.linalg.norm(points - center, axis=1) + support[indices]))
            local_cap = min(global_cap, float(np.max(support[indices])) + geometric_budget)
            if required > local_cap + 1e-12 and len(indices) > 1:
                spread = np.ptp(points, axis=0)
                if float(np.max(spread)) <= 1e-15:
                    split_order = np.sort(indices)
                else:
                    axis = int(np.argmax(spread))
                    split_order = indices[np.argsort(points[:, axis], kind='stable')]
                midpoint = len(split_order) // 2
                if 0 < midpoint < len(split_order) and depth < 32:
                    refine(split_order[:midpoint], depth + 1)
                    refine(split_order[midpoint:], depth + 1)
                    return
            if required > global_cap + 1e-09:
                raise ValueError('resolution-bounded sphere candidate exceeded global cap')
            candidate_centers.append(center)
            candidate_radii.append(required)
        for group in groups:
            refine(np.asarray(group, dtype=np.int64))
        return (np.asarray(candidate_centers, dtype=float), np.asarray(candidate_radii, dtype=float))
    ...
    def update(self, points: np.ndarray, point_radii: np.ndarray, point_uncertainty_shapes: np.ndarray | None=None) -> ProxyUpdateStats:
        ...
        if self.sphere_cover_mode == 'adaptive_irredundant':
            candidates = self._fit_incremental_snapshot(ordered_keys, changed, directional)
            candidate_centers, candidate_radii = self._build_resolution_bounded_sphere_candidates(ordered_keys, candidates)
            proxies = self._publish_adaptive_irredundant_sphere_snapshot(ordered_keys, candidate_centers, candidate_radii)
        else:
        ...
        return ProxyUpdateStats(generation=self._generation, inserted_points=len(points), center_voxels=len(filtered), proxy_count=len(proxies.centers), changed_center_voxels=len(changed), update_ms=float(elapsed_ms), snapshot_sha256=self._snapshot_hash, frame_centervox_ms=float((frame_centervox_done - started) * 1000.0), history_union_ms=float((history_union_done - frame_centervox_done) * 1000.0), cell_merge_ms=float((cell_merge_done - history_union_done) * 1000.0), bucket_fit_ms=float((bucket_fit_done - cell_merge_done) * 1000.0), publish_ids_hash_ms=float((time.perf_counter() - bucket_fit_done) * 1000.0), sphere_cover_candidate_count=proxies.sphere_cover_candidate_count, sphere_cover_selected_count=proxies.sphere_cover_selected_count, sphere_cover_reverse_deleted=proxies.sphere_cover_reverse_deleted)
    ...
    def _adaptive_sphere_coverage_audit(self, tolerance: float) -> ProxyCoverageAudit:
        proxies = self.snapshot
        keys = self._last_ordered_keys
        ...
        if len(owner) != len(cells) or np.any(owner < 0) or np.any(owner >= len(proxies.centers)):
        ...
        required = np.linalg.norm(assigned_centers - cell_centers, axis=1) + cell_support_radii
        published = np.asarray(proxies.sphere_radii, dtype=float)[owner]
        slacks = published - required
        minimum_slack = float(np.min(slacks))
        ratios = required / np.maximum(published, 1e-18)
        ...
        for center, radius in zip(proxies.centers, proxies.sphere_radii):
            all_required = np.linalg.norm(np.asarray(center, dtype=float)[None, :] - cell_centers, axis=1) + cell_support_radii
            covered = np.flatnonzero(all_required <= float(radius) + tolerance)
            per_proxy_coverage.append(covered)
            cover_counts[covered] += 1
        complete = bool(np.all(cover_counts >= 1))
        inclusion_minimal = bool(complete and all((np.any(cover_counts[covered] == 1) for covered in per_proxy_coverage)))
        cap_ok = bool(self.certificate_radius_limit is not None and np.all(np.asarray(proxies.sphere_radii, dtype=float) <= self.certificate_radius_limit + tolerance))
        raw_source_count = sum((int(sample.source_count) for cell in cells for sample in cell.samples.values()))
        certified = bool(minimum_slack >= -tolerance and complete and inclusion_minimal and cap_ok)
        return ProxyCoverageAudit(raw_sample_bins=int(raw_source_count), center_voxels=len(cells), proxy_count=len(proxies.centers), minimum_centervox_cover_slack=minimum_slack, minimum_proxy_offset_slack=0.0, maximum_base_ellipsoid_value=float(np.max(np.square(ratios))), minimum_sphere_certificate_slack=minimum_slack, all_raw_sample_balls_certified=certified, minimum_measurement_uncertainty_slack=minimum_slack, minimum_proxy_uncertainty_slack=0.0, sphere_cover_inclusion_minimal=inclusion_minimal, sphere_cover_candidate_count=proxies.sphere_cover_candidate_count, sphere_cover_selected_count=proxies.sphere_cover_selected_count, sphere_cover_reverse_deleted=proxies.sphere_cover_reverse_deleted)
