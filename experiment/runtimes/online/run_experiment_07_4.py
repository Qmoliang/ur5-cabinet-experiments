"""07.4 development runs; no changes to frozen 07.2 artifacts."""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--smoke',action='store_true');ap.add_argument('--groups',nargs='+',default=['ET30','D30'],choices=['ET30','D30','P30']);ap.add_argument('--seeds',nargs='+',type=int,default=list(range(5)));ap.add_argument('--with-pilot',action='store_true');args=ap.parse_args()
    batch=ROOT/'formal_results/experiment_07/development_07_4'/('smoke_' if args.smoke else 'online_')/datetime.now().strftime('%Y%m%d_%H%M%S');batch.mkdir(parents=True,exist_ok=False)
    sources=['model.py','official_drawer_07_3.py','raw_support_intervals.py','raw_directional_uncertainty.py','incremental_proxy_manager.py','pointcloud_proxy.py','protocol_liuqp_controller.py','run_protocol_v3_async_online.py','run_experiment_07_4.py','plan/experiment-07-4-protocol.md']
    ...
    manifest=dict(experiment='07.4',development_only=True,mock_data=False,command=sys.argv,python=sys.version,platform=platform.platform(),protocol='plan/experiment-07-4-protocol.md',sources_sha256=hashes,results=[])
    ...
    for seed in args.seeds:
        ...
        groups=list(args.groups if seed%2==0 else args.groups[::-1])
        if args.with_pilot and seed==0 and 'P30' not in groups:groups.append('P30')
        for group in groups:
            mode={'ET30':'separate_uncertainty','D30':'raw_directional_uncertainty','P30':'partitioned_separate_uncertainty'}[group]
