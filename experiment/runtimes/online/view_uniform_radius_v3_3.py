"""Open the formal 30/50/70 mm online comparison in the MuJoCo viewer."""
...
def _run_directory(representation: str, scale_mm: int) -> Path:
    matches = sorted(
        path
        for path in RESULTS.iterdir()
        if path.is_dir()
        and f"-{representation}-" in path.name
        and f"-cr{scale_mm}mm-" in path.name
    )
    if len(matches) != 1:
        raise RuntimeError(
            f"expected one {representation} {scale_mm} mm run, found {len(matches)}"
        )
    return matches[0]
...
def main() -> None:
    ...
    parser.add_argument("--scale", type=int, choices=(30, 50, 70), default=50)
