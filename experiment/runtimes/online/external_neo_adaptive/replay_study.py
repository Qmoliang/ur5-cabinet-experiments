"""Causal proxy replay for the six actual online trials, including partial failure."""
...
def main():
    ...
    if args.check:
        ...
        print('PASS: six replays load, including the aborted partial run; proxies selected causally.');return
    if args.render:
        ...
        print('Six actual final/partial-state images rendered.');return
    commands=queue.Queue()
    def key(k):
        if 0<=k<128:commands.put(chr(k).upper())
    # All trials use byte-identical scenes, so one viewer can replay them.
    r=results[0];model=r['model'];data=r['data'];case=0;frame=0;paused=False;show=False;show_robot=False;last=time.perf_counter()
    with mujoco.viewer.launch_passive(model,data,key_callback=key) as viewer:
        camera(viewer.cam)
        while viewer.is_running():
            while not commands.empty():
                c=commands.get_nowait()
                if c in '123456':case=int(c)-1;frame=0;paused=False
            ...
            text+='1 NEO sphere300 | 2 NEO ellipsoid300 | 3 NEO sphere46 (aborted)\n4 NEO ellipsoid46 | 5 Liu sphere | 6 Liu ellipsoid\n'
            text+='Space pause | 0 restart | F final | V causal proxies | R robot balls\n'
