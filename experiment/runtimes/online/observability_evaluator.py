"""Read-only causal observability audit for completed local-depth runs.

This module may read MuJoCo truth, but it is deliberately not a controller
dependency.  The online runner records immutable observed-point snapshots and
calls this evaluator only after the control loop has ended.
"""
...
from pointcloud_proxy import PointCloudProxySet, sample_box_surface_point_cloud
...
class ObservabilityReport:
    ...
    never_observed_sample_bounds_by_box_m: dict[
...
def snapshot_from_proxies(
    ...
    *,
    ...
    """Copy the actually observed representatives and their cover bounds."""
    ...
    if proxies.filtered_uncertainty_shapes is None:
        ...
        shapes = np.asarray(
            proxies.filtered_uncertainty_shapes, dtype=float
        ).reshape(-1, 3, 3)
    ...
    # CenterVox retains one real observed point per fixed world cell.  Any raw
    # observed center assigned to it is at most one voxel diagonal away.
    center_cover = math.sqrt(3.0) * float(centervox_size)
    cover = measurement + center_cover
    return ObservedPointSnapshot(
        publish_cycle=int(publish_cycle),
        source_cycle=int(source_cycle),
        points=points.copy(),
        cover_radii=cover.copy(),
    )


def load_observability_snapshots(path: str) -> list[ObservedPointSnapshot]:
    """Load the lossless ragged snapshot archive emitted by the online runner."""
    ...
    return [
        ObservedPointSnapshot(
            ...
            source_cycle=int(source[index]),
            points=points[offsets[index] : offsets[index + 1]].copy(),
            cover_radii=cover[offsets[index] : offsets[index + 1]].copy(),
        )
        for index in range(len(publish))
    ]


def _first_seen_cycles(
    truth_points: np.ndarray,
    snapshots: list[ObservedPointSnapshot],
) -> np.ndarray:
    ...
    for snapshot in sorted(snapshots, key=lambda item: item.publish_cycle):
        if not len(snapshot.points):
            continue
        neighborhoods = truth_tree.query_ball_point(
            np.asarray(snapshot.points, dtype=float),
            np.asarray(snapshot.cover_radii, dtype=float),
            workers=-1,
        )
...
def _sample_truth_surfaces(
    boxes: tuple[BoxObstacle, ...], spacing: float
) -> tuple[np.ndarray, list[BoxSurfaceRange]]:
    """Sample each solid separately so occluded opposite faces stay identifiable."""
...
def _nearest_surface_distance(
    ...
    """Distance to the relevant boundary face, including centers inside a solid."""
...
def _nearest_surface_patch_indices(
    center: np.ndarray,
    box: BoxObstacle,
    ...
    """Return only the sampled patch nearest the query point on this solid.

    A thin box has two nearby Euclidean surface sheets.  Only the sheet attaining
    the point-to-solid distance is a current collision witness; the opposite
    sheet may be physically occluded by the solid itself and is not required.
    """
    ...
    # Sampling roundoff must not erase a genuine witness.
    _, nearest = tree.query(center, k=1)
    return np.asarray([int(nearest)], dtype=np.int64)


def _filter_risk_patch_indices(
    center: np.ndarray,
    radius: float,
    box_points: np.ndarray,
    indices: np.ndarray,
    ...
    """Apply the one allowed surface-sampling correction to risk witnesses.

    Point-to-box distance is analytic, while ``box_points`` discretizes the
    continuous closest face.  The correction therefore belongs only here,
    when a continuous face witness is mapped to a sampled point.  Filtering
    prevents the broadphase correction and the patch correction from being
    added twice at an edge/corner.
    """
    ...
    distances = np.linalg.norm(
        np.asarray(box_points, dtype=float)[indices]
    ...
    limit = (
        ...
        + float(sampling_correction)
        + 1.0e-12
    )
    return indices[distances <= limit]


def _first_risk_cycles(
    scene: SceneDefinition,
    q_trajectory: np.ndarray,
    truth_points: np.ndarray,
    box_ranges: list[BoxSurfaceRange],
    observation_distance: float,
    lipschitz_correction: float,
) -> np.ndarray:
    ...
    trees = [
        cKDTree(truth_points[item.start : item.stop]) for item in box_ranges
    ]
    lowers = np.asarray(
        [
            np.asarray(item.box.center) - np.asarray(item.box.half_size)
    ...
    for cycle, q in enumerate(np.asarray(q_trajectory, dtype=float)):
        ...
        for center, radius in zip(centers, radii):
            outside_delta = np.maximum(
                np.maximum(lowers - center, center - uppers), 0.0
            )
            solid_distances = np.linalg.norm(outside_delta, axis=1)
            risk_boxes = np.flatnonzero(
                solid_distances <= float(radius) + query_extra
            )
            for box_index in risk_boxes:
                item = box_ranges[int(box_index)]
                local = _nearest_surface_patch_indices(
                    center,
                    item.box,
                    truth_points[item.start : item.stop],
                    trees[int(box_index)],
                    lipschitz_correction,
                )
                local = _filter_risk_patch_indices(
                    center,
                    float(radius),
                    truth_points[item.start : item.stop],
                    local,
...
def evaluate_observed_before_risk(
    scene: SceneDefinition,
    q_trajectory: np.ndarray,
    snapshots: list[ObservedPointSnapshot],
    *,
    observation_distance: float = 0.10,
    guard_time: float = 0.10,
    truth_surface_spacing: float = 0.012,
) -> ObservabilityReport:
    """Audit causal observation timing without modifying a completed run."""
    ...
    truth, box_ranges = _sample_truth_surfaces(
        scene.boxes, spacing=float(truth_surface_spacing)
    )
    ...
    if np.any(violation):
        ...
        if np.any(finite_late):
            maximum_lateness = int(
                np.max(seen[finite_late] - deadlines[finite_late])
            )
    ...
    for item in box_ranges:
        at_risk_by_box[item.box.name] = int(
            np.count_nonzero(risk_mask[item.start : item.stop])
        )
        violations_by_box[item.box.name] = int(
            np.count_nonzero(violation_global_mask[item.start : item.stop])
        )
        late_by_box[item.box.name] = int(
            np.count_nonzero(late_global_mask[item.start : item.stop])
        )
        never_by_box[item.box.name] = int(
            np.count_nonzero(never_global_mask[item.start : item.stop])
        )
        ...
        late_bounds_by_box[item.box.name] = (
            None
            if not len(box_late)
            else {
                "minimum": np.min(box_late, axis=0).tolist(),
                "maximum": np.max(box_late, axis=0).tolist(),
            }
        )
        never_bounds_by_box[item.box.name] = (
            None
            if not len(box_never)
            else {
                "minimum": np.min(box_never, axis=0).tolist(),
                "maximum": np.max(box_never, axis=0).tolist(),
            }
        )
    return ObservabilityReport(
        audit_method="nearest_solid_surface_witness_v2",
        passed=bool(passed),
        event_code="observed_before_risk" if passed else "unobserved_hazard",
        observation_distance_m=float(observation_distance),
        guard_time_s=float(guard_time),
        guard_cycles=guard_cycles,
        truth_surface_spacing_m=float(truth_surface_spacing),
        truth_lipschitz_correction_m=float(correction),
        truth_surface_samples=int(len(truth)),
        ever_at_risk_samples=int(len(risk_indices)),
        observed_before_deadline_samples=int(np.count_nonzero(valid)),
        late_samples=int(np.count_nonzero(late)),
        never_observed_samples=int(np.count_nonzero(never)),
        maximum_observation_lateness_cycles=maximum_lateness,
        first_violation_cycle=first_violation_cycle,
        first_violation_point_m=first_violation_point,
        first_violation_box=first_violation_box,
        first_violation_kind=first_violation_kind,
        first_violation_seen_cycle=first_violation_seen_cycle,
        first_violation_deadline_cycle=first_violation_deadline_cycle,
        at_risk_samples_by_box=at_risk_by_box,
        violation_samples_by_box=violations_by_box,
        late_samples_by_box=late_by_box,
        never_observed_samples_by_box=never_by_box,
        late_sample_bounds_by_box_m=late_bounds_by_box,
        never_observed_sample_bounds_by_box_m=never_bounds_by_box,
