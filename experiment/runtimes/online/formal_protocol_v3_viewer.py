"""MuJoCo replay for the final causal sphere/ellipsoid LiuQP comparison.

The replay uses the saved joint trajectory, causal observed-point snapshots,
and the exact immutable proxy snapshot that was active at each control cycle.
No proxy is shown before its recorded publication cycle.
"""
...
ELLIPSOID_RGBA = np.array([1.00, 0.55, 0.02, 0.20], dtype=np.float32)
OUTER_RGBA = np.array([1.00, 0.92, 0.12, 0.065], dtype=np.float32)
UNCERTAINTY_RGBA = np.array([0.70, 0.30, 1.00, 0.10], dtype=np.float32)
ROBOT_PROXY_RGBA = np.array([0.05, 0.78, 1.00, 0.20], dtype=np.float32)
POINT_RGBA = np.array([0.15, 0.95, 0.42, 0.72], dtype=np.float32)
FREE_RGBA = np.array([0.12, 0.52, 1.00, 0.10], dtype=np.float32)
OCCUPIED_RGBA = np.array([0.95, 0.08, 0.18, 0.24], dtype=np.float32)
CANDIDATE_RGBA = np.array([0.05, 0.90, 0.95, 0.28], dtype=np.float32)
ACTIVE_RGBA = np.array([0.95, 0.05, 0.78, 0.48], dtype=np.float32)
TARGET_RGBA = np.array([0.20, 0.95, 0.20, 0.90], dtype=np.float32)
LIMIT_ROBOT_RGBA = np.array([1.00, 0.02, 0.45, 0.92], dtype=np.float32)
LIMIT_SAFETY_RGBA = np.array([1.00, 0.02, 0.45, 0.16], dtype=np.float32)
LIMIT_OBSTACLE_RGBA = np.array([1.00, 0.90, 0.02, 0.88], dtype=np.float32)
LIMIT_SUPPORT_RGBA = np.array([1.00, 1.00, 1.00, 0.98], dtype=np.float32)
LIMIT_SAFE_SEGMENT_RGBA = np.array([1.00, 0.05, 0.05, 0.98], dtype=np.float32)
LIMIT_REMAINING_RGBA = np.array([0.15, 1.00, 0.30, 0.98], dtype=np.float32)
LIMIT_VIOLATION_RGBA = np.array([1.00, 0.05, 0.05, 0.98], dtype=np.float32)
LIMIT_RECOVERY_RGBA = np.array([1.00, 0.28, 0.02, 0.98], dtype=np.float32)
...
def _append_geom(scene, geom_type, size, position, rotation, rgba) -> None:
    if scene.ngeom >= scene.maxgeom:
        # The physical MuJoCo scene is never affected.  If a user enables all
        # optional diagnostic layers at once, cap only the overlay instead of
        # crashing the replay; core proxies are inserted before optional U/map.
        return
...
def _append_connector(scene, start, stop, rgba, width: float = 6.0) -> None:
    """Draw a diagnostic line without changing the physical MuJoCo model."""
...
class RaggedSnapshots:
    def __init__(self, path: Path) -> None:
        ...
        self.arrays = {
            ...
            if name not in {"publish_cycles", "source_cycles", "offsets"}
        }
            ...
            for row in csv.DictReader(stream):
                ...
                if row.get("clearance") not in (None, ""):
                    ...
                    if current is None or clearance < float(current["clearance"]):
                        self.minimum_by_cycle[cycle] = {
                            "clearance": clearance,
                            "surface_clearance": float(
                                row.get("surface_clearance", clearance + 0.006)
                            ),
                            "state": row.get("state", "unknown"),
                            "robot_index": int(row.get("robot_index", -1)),
                            "obstacle_index": int(row.get("obstacle_index", -1)),
                            "proxy_id": int(row["proxy_id"]),
                            "contact_recovery_row": row.get(
                                "contact_recovery_row", "False"
                            ).lower()
                            == "true",
                        }
...
class EmptyCycleProxySelection:
    """Replay fallback when broadphase candidate identities were not persisted."""
...
class OccupancyDeltaSnapshots:
    """Lazy sparse-map replay; absent keys are exactly UNKNOWN."""
    ...
    def __init__(self, path: Path) -> None:
        ...
        self.delta_keys = (
            shifted[:, 0]
            | (shifted[:, 1] << np.uint64(21))
            | (shifted[:, 2] << np.uint64(42))
        )
...
class FormalRun:
    def __init__(self, directory: Path, *, replay_only: bool = False) -> None:
        ...
        self.points = RaggedSnapshots(
            directory / "causal_observability_snapshots.npz"
        )
        ...
        self.occupancy = OccupancyDeltaSnapshots(
            recorded_occupancy
            if recorded_occupancy.exists()
            else replayed_occupancy
        )
        ...
        self.candidate_report = (
            json.loads(candidate_report.read_text(encoding="utf-8"))
            if candidate_report.exists()
            else None
        )
        self.sweep_report = (
            json.loads(sweep_report.read_text(encoding="utf-8"))
        ...
        occupancy_report = (
            recorded_report if recorded_report.exists() else legacy_report
        )
        self.occupancy_report = (
            json.loads(occupancy_report.read_text(encoding="utf-8"))
            if occupancy_report.exists()
            else None
        )
        if not replay_only and (
            self.candidate_report is None
            or self.sweep_report is None
            or self.occupancy_report is None
        ):
            raise FileNotFoundError(
                f"formal audit sidecars are incomplete below {directory}"
            )
        if len(self.q) != len(self.cycles):
            raise ValueError(f"q/cycle length mismatch: {directory}")
        if self.summary["causal_proxy_snapshots_saved"] != len(
            self.proxies.publish_cycles
        ):
...
class FormalComparison:
    def __init__(
        self, sphere_dir: Path, ellipsoid_dir: Path, *, replay_only: bool = False
    ) -> None:
        ...
        if sphere_dir.resolve() == ellipsoid_dir.resolve():
            ...
            self.target = np.asarray(
                _protocol_scene(run.summary["scene_version"]).waypoints[-1],
                dtype=float,
            )
            return
        self.runs = {
            "sphere": FormalRun(sphere_dir, replay_only=self.replay_only),
            "ellipsoid": FormalRun(ellipsoid_dir, replay_only=self.replay_only),
        }
        self.locked_mode = None
        sphere = self.runs["sphere"]
        ellipsoid = self.runs["ellipsoid"]
        if _sha256(sphere_dir / "scene.xml") != _sha256(
            ellipsoid_dir / "scene.xml"
        ):
            raise ValueError("sphere and ellipsoid scene XML hashes differ")
        if (
            not self.replay_only
            and sphere.summary["source_hashes"] != ellipsoid.summary["source_hashes"]
        ):
        ...
        self.robot_spheres = build_robot_certificate(self.model)
        self.target = np.asarray(
            _protocol_scene(sphere.summary["scene_version"]).waypoints[-1],
            dtype=float,
        )
    ...
    def populate(
        ...
        *,
        ...
        drawn_obstacles = 0
        if mode == "sphere":
            for proxy_id, center, radius, offset in zip(
                proxies["proxy_ids"],
                proxies["centers"],
                proxies["sphere_radii"],
                proxies["uncertainty_offsets"],
            ):
                ...
                rgba = (
                    ACTIVE_RGBA
                    if selected_active
                    else CANDIDATE_RGBA if selected_candidate else SPHERE_RGBA
                )
                _append_geom(
                    scene,
                    mujoco.mjtGeom.mjGEOM_SPHERE,
                    np.full(3, float(radius + offset)),
                    center,
                    IDENTITY,
                    rgba,
                )
            ...
            if show_outer and not (show_core and outer_matches_core):
                for proxy_id, center, shape in zip(
                    proxies["proxy_ids"],
                    proxies["centers"],
                    proxies["ellipsoid_outer_shapes"],
                ):
                    ...
                    _append_geom(
                        ...
                        ACTIVE_RGBA
                        if selected_active
                        else CANDIDATE_RGBA if selected_candidate else OUTER_RGBA,
                    )
                    drawn_obstacles += 1
            if show_core:
                for proxy_id, center, shape in zip(
                    ...
                    _append_geom(
                        ...
                        ACTIVE_RGBA
                        if selected_active
                        else CANDIDATE_RGBA if selected_candidate else ELLIPSOID_RGBA,
                    )
                    drawn_obstacles += 1
            if show_uncertainty:
                for proxy_id, center, shape in zip(
        ...
        if show_map:
            ...
            for state, rgba in ((1, FREE_RGBA), (2, OCCUPIED_RGBA)):
                ...
                for position in OccupancyDeltaSnapshots.centers(
                    sample, run.occupancy.voxel_size
                ):
                    _append_geom(
                        scene,
        ...
        if show_robot or show_limiting:
            robot_positions, _, robot_radii = certificate_world_state(
        ...
        # Highlight the exact minimum-clearance pair recorded by the online
        # controller, rather than a hand-picked robot sphere.
        ...
        if limiting_record is not None:
            ...
            limiting_logged_surface_clearance = float(
                limiting_record["surface_clearance"]
            )
            limiting_contact_recovery = bool(
                limiting_record["contact_recovery_row"]
            )
        ...
        if (
            show_limiting
            and robot_positions is not None
            and 0 <= limiting_robot_index < len(robot_positions)
            and limiting_proxy_id >= 0
        ):
            proxy_ids = np.asarray(proxies["proxy_ids"], dtype=np.int64)
            if not (
                0 <= limiting_obstacle_index < len(proxy_ids)
                and int(proxy_ids[limiting_obstacle_index]) == limiting_proxy_id
            ):
            ...
            if 0 <= limiting_obstacle_index < len(proxy_ids):
                ...
                obstacle_offset = float(
                    proxies["uncertainty_offsets"][limiting_obstacle_index]
                )
                limiting_robot_body = self.robot_spheres[
                    limiting_robot_index
                ].body_name
                if mode == "sphere":
                    obstacle_radius = float(
                        proxies["sphere_radii"][limiting_obstacle_index]
                    ) + obstacle_offset
                    ...
                    _append_geom(
                        scene,
                        mujoco.mjtGeom.mjGEOM_SPHERE,
                        np.full(3, obstacle_radius),
                        obstacle_center,
                        IDENTITY,
                        LIMIT_OBSTACLE_RGBA,
                    )
                else:
                    obstacle_shape = proxies["ellipsoid_shapes"][
                        limiting_obstacle_index
                    ]
                    uncertainty_shape = proxies["proxy_uncertainty_shapes"][
                        limiting_obstacle_index
                    ]
                    support = optimal_support_normal_with_uncertainty(
                        robot_center,
                        obstacle_center,
                        obstacle_shape,
                        uncertainty_shape,
                    ...
                    obstacle_surface = (
                        support.obstacle_surface_point - obstacle_offset * normal
                    )
                ...
                _append_geom(
                    scene,
                    mujoco.mjtGeom.mjGEOM_SPHERE,
                    np.full(3, robot_radius + safety_margin),
                    robot_center,
                    IDENTITY,
                    LIMIT_SAFETY_RGBA,
                )
                for point, rgba in (
                    (robot_surface, LIMIT_ROBOT_RGBA),
                    (safety_boundary, LIMIT_SAFE_SEGMENT_RGBA),
                    (obstacle_surface, LIMIT_SUPPORT_RGBA),
                ):
                    _append_geom(
                        scene,
                        mujoco.mjtGeom.mjGEOM_SPHERE,
                ...
                _append_connector(
                    ...
                    obstacle_surface,
                    LIMIT_RECOVERY_RGBA
                    if limiting_contact_recovery
                    else (
                        LIMIT_REMAINING_RGBA
                        if limiting_reconstructed_clearance >= 0.0
                        else LIMIT_VIOLATION_RGBA
                    ),
                    width=8.0,
                )
        ...
        return {
            "proxy_snapshot": proxy_index,
            "proxy_publish_cycle": int(run.proxies.publish_cycles[proxy_index]),
            "proxy_source_cycle": int(run.proxies.source_cycles[proxy_index]),
            "proxy_count": len(proxies["centers"]),
            "point_snapshot": point_index,
            "observed_point_count": len(observed["points"]),
            "drawn_points": drawn_points,
            "drawn_obstacles": drawn_obstacles,
            "drawn_robot": drawn_robot,
            "candidate_proxy_count": len(candidate_ids),
            "active_proxy_count": len(active_ids),
            "map_snapshot": map_index,
            "free_voxel_count": free_count,
            "occupied_voxel_count": occupied_count,
            "drawn_free_voxels": drawn_free,
            "drawn_occupied_voxels": drawn_occupied,
            "limiting_pair_drawn": limiting_pair_drawn,
            "limiting_robot_index": limiting_robot_index,
            "limiting_obstacle_index": limiting_obstacle_index,
            "limiting_proxy_id": limiting_proxy_id,
            "limiting_robot_body": limiting_robot_body,
            "limiting_logged_clearance_m": limiting_logged_clearance,
            "limiting_logged_surface_clearance_m": (
                limiting_logged_surface_clearance
            ),
            "limiting_state": limiting_state,
            "limiting_contact_recovery": limiting_contact_recovery,
            "limiting_reconstructed_clearance_m": limiting_reconstructed_clearance,
            "limiting_raw_gap_m": limiting_raw_gap,
    ...
    def check(self) -> None:
        ...
        for mode, run in self.runs.items():
            if not self.replay_only:
                ...
                assert run.candidate_report[
                    "all_per_cycle_candidate_counts_matched"
                ]
                ...
                if "all_online_frame_state_counts_matched" in run.occupancy_report:
                    assert run.occupancy_report[
                        "all_online_frame_state_counts_matched"
                    ]
                else:
                    assert run.occupancy_report[
                        "cycles_match_source_proxy_and_frame_logs"
                    ]
                assert run.occupancy_report["snapshot_count"] == len(
                    run.occupancy.publish_cycles
                )
            ...
            assert np.all(
                run.occupancy.source_cycles <= run.occupancy.publish_cycles
            )
            if run.candidate_selection_available:
                assert all(
                    run.active.at_cycle(cycle) <= run.candidates.at_cycle(cycle)
                    for cycle in range(len(run.cycles))
                )
            ...
            print(
                f"{mode}: cycles={len(run.cycles)}, "
                f"proxy_snapshots={len(run.proxies.publish_cycles)}, "
                f"point_snapshots={len(run.points.publish_cycles)}, "
                f"success={run.summary['success']}, "
                f"deadline_misses={run.summary['deadline_misses']}"
            )
        if self.replay_only:
            print(
                "replay-only load verified: recorded trajectory, causal points, "
                "proxies, occupancy, and QP-active rows are readable; missing "
                "formal sidecars were not claimed as audited"
            )
            return
        prefix = (
            "single-run artifacts loaded; "
            if self.locked_mode is not None
            else "scene/source hashes match; "
        )
        print(
            prefix
            + "causal publication ordering, candidate replay, active-subset, "
            "occupancy replay, and unmodified continuous sweep audit verified"
        )
    ...
    def run(self, initial: str, speed: float) -> None:
        ...
        if any(
            not run.candidate_selection_available for run in self.runs.values()
        ):
            print(
                "Replay note: this result did not persist broadphase candidate IDs; "
                "B is empty, while O and Q remain exact recorded overlays."
            )
        ...
        with mujoco.viewer.launch_passive(
            ...
            # Hide collision duplicates only; visual UR5 and drawer geoms stay on.
            viewer.opt.geomgroup[1] = 0
            if any(run.summary.get("scene_version") == "exp07_drawer_official" for run in self.runs.values()):
                viewer.opt.geomgroup[5] = 1
            viewer.cam.lookat[:] = np.array([0.42, 0.40, 0.52])
            ...
            while viewer.is_running():
                    ...
                    if char == "1" and "sphere" in self.runs:
                        ...
                        cycle = int(
                            np.argmin(
                                [
                                    float(item["error_m"])
                                    for item in self.runs[mode].cycles
                                ]
                            )
                        )
                        paused = True
                ...
                with viewer.lock():
                    ...
                    counts = self.populate(
                        ...
                        mode,
                        cycle,
                        show_points=overlays_visible and show_points,
                        show_obstacles=overlays_visible and show_obstacles,
                        show_robot=overlays_visible and show_robot,
                        show_core=show_core,
                        show_outer=show_outer,
                        show_uncertainty=show_uncertainty,
                        show_map=overlays_visible and show_map,
                        show_candidates=overlays_visible and show_candidates,
                        show_active=overlays_visible and show_active,
                        show_limiting=overlays_visible and show_limiting,
                    )
                label = (
                    f"{mode} LiuQP — "
                    + ("reached target" if run.summary["success"] else "did not reach target")
                )
                viewer.set_texts(
                    (
                        ...
                        (
                            ...
                            f"error={1000*float(row['error_m']):.1f} mm  "
                            f"QP={row['status']}  speed={playback_speed:g}x\n"
                            f"causal proxy gen={counts['proxy_snapshot']} "
                            f"publish={counts['proxy_publish_cycle']} "
                            f"source={counts['proxy_source_cycle']} "
                            f"proxies={counts['proxy_count']}\n"
                            f"observed CenterVox={counts['observed_point_count']} "
                            f"(drawn={counts['drawn_points']})  "
                            f"robot spheres={counts['drawn_robot']}\n"
                            f"AABB candidate proxies={counts['candidate_proxy_count']}  "
                            f"final QP proxies={counts['active_proxy_count']}\n"
                            f"limit pair=R{counts['limiting_robot_index']} "
                            f"({counts['limiting_robot_body']}) <-> "
                            f"P{counts['limiting_proxy_id']}  "
                            f"state={counts['limiting_state']}  "
                            f"c(log)={1000*counts['limiting_logged_surface_clearance_m']:.2f} mm  "
                            f"h(log)={1000*counts['limiting_logged_clearance_m']:.2f} mm  "
                            f"h(recon)={1000*counts['limiting_reconstructed_clearance_m']:.2f} mm\n"
                            f"exact proxy surface gap={1000*counts['limiting_raw_gap_m']:.2f} mm  "
                            f"hard safety={1000*counts['safety_margin_m']:.1f} mm  "
                            f"MuJoCo penetrating contacts={row['exact_penetrating_contact_count']}\n"
                            f"map snapshot={counts['map_snapshot']}  "
                            f"FREE={counts['free_voxel_count']}  "
                            f"OCCUPIED={counts['occupied_voxel_count']}  "
                            "UNKNOWN=all absent bounded-grid keys\n"
                            "Fused ellipsoid QP uses exact point-to-E(c,Q) gap; "
                            "U=0 and offset=0, with no post-core inflation.\n"
                            "1/2 mode | V overlays | P points | O all proxies | "
                            "B candidates | Q QP-active | M map | R robot balls | H limiting pair"
                        ),
                    )
                )
                viewer.sync()
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "allow visual replay of compatible frozen runs when optional formal "
            "audit sidecars or matching source hashes are unavailable"
        ),
    )
